...
$reservationQuery = "
    ...
    WHERE r.codeR = ? AND r.etat = 'pas encore traité'
";
$reservationStmt = $mysqli->prepare($reservationQuery);
if (!$reservationStmt) {
    die("Error preparing reservation query: " . $mysqli->error);
}
$reservationStmt->bind_param("i", $codeR);
$reservationStmt->execute();
$reservationResult = $reservationStmt->get_result();
if (!$reservationResult) {
    die("Error executing reservation query: " . $mysqli->error);
}
$reservation = $reservationResult->fetch_assoc();
if (!$reservation) {
    die("No reservation found for codeR = $codeR or reservation already processed");
}
...
if (!$servicesStmt) {
    die("Error preparing services query: " . $mysqli->error);
}
$servicesStmt->bind_param("i", $codeR);
$servicesStmt->execute();
$services = $servicesStmt->get_result();
if (!$services) {
    die("Error executing services query: " . $mysqli->error);
}
